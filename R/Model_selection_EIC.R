...
#' @export
vbgamlss.model_selection_EIC <- function(# model selection commands
  ...
  if (is.null(resume_registry)) {
    ...
    for (i in 1:nfm) {
      ...
      CALL <- glue("
                   ...
                   setwd('{slurm$wd}')
                   out <- vbgamlss.evaluate(imageframe         = imageframe,
                                            g.formula          = g.formula,
                                            data               = dtfr,
                                            g.family           = g.family,
...
slurm_resources <- function(n='VBGAMLSS', o=NULL, e=NULL,
  ...
  slurm$rdsout    <- as.character(r)     # path to the results qs file
...
# -----------------------------------------------------------
slurm_registry <- function(Njobs, env=NULL, run_name="eval_run"){
  registry              <- list()
  registry$wd           <- getwd()
  ...
  registry$jobs_sout    <- file.path(registry$jobspaths, paste0(registry$jobs, '.stdout'))
  registry$jobs_results <- file.path(registry$jobspaths, paste0(registry$jobs, '.results.qs'))
  registry$jobs_script  <- file.path(registry$jobspaths, paste0(registry$jobs, '.R'))
  registry$jobs_sbatch  <- file.path(registry$jobspaths, paste0(registry$jobs, '.job'))
  registry$jobs_env     <- file.path(registry$jobspaths, paste0(registry$jobs, '.local.env'))
...
jobs_status <- function(registry, param='status') {
  ...
  for (job in registry$jobs_id){
    status_ <- system(glue('sacct -j {job} --format="JobID,State,ExitCode,Elapsed,End"'), intern = TRUE)
...
# -----------------------------------------------------------
#' @export
cleanup_cached_voxels <- function(resume_registry, dry_run = TRUE) {
  ...
  for (i in seq_len(n_jobs)) {
    ...
    # Locate base cache directories directly in the job path (CV states are gone)
    cache_dirs <- list.dirs(job_dir, recursive = FALSE, full.names = TRUE)
    cache_dirs <- cache_dirs[grepl("\\.vbgamlss\\.cache", basename(cache_dirs))]

    if (length(cache_dirs) == 0) {
      cat("\t| No vbgamlss caches found, skipping.\n")
      next
    }
